...
class Reflist():
	...
	def handleArticle(self, wiki, article):
		pageText = WikipediaAPI.getPageText(wiki, article)

		if not pageText:
			return {'status':'error', 'message':'Could not get page text'}
			
		if self.lacksReferences(pageText):
			newText = self.addReferences(pageText)
			
			return {'status':'ok', 'title':article,'origText':pageText,'changedText':newText}
		
		return {'status':'error','message':'Article already has reference tag'}
